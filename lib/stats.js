...
import { dayEnd, dayStart, shift, startOfDay, startOfMonth, startOfWeek } from "./time";
...
/** A hand-picked range travels as one string: "custom:2026-09-01:2026-09-20". */
export const CUSTOM = "custom";
export const customRange = (from, to) => `${CUSTOM}:${from || ""}:${to || ""}`;
export const isCustom = (range) => String(range || "").startsWith(`${CUSTOM}:`);
export function customDays(range) {
  const [, from = "", to = ""] = String(range || "").split(":");
  return { from, to };
}

/**
 * The real-UTC instants a range spans — `from` null means "since the beginning",
 * `to` null means "up to now". Both ends are cut on the market clock: today
 * starts at the market's midnight (the trading-day rollover) and the week at its
 * Monday, not at the viewer's. The rolling windows (7d/30d/90d) count back from
 * right now, and a hand-picked range covers whole market days, ends included.
 */
export function rangeBounds(range, now = new Date(), offsetMin = 0) {
  const none = { from: null, to: null };
  if (!range || range === "all") return none;
  if (isCustom(range)) {
    const { from, to } = customDays(range);
    return { from: from ? dayStart(from, offsetMin) : null, to: to ? dayEnd(to, offsetMin) : null };
  }
  if (range === "today") return { from: startOfDay(now, offsetMin), to: null };
  if (range === "wtd") return { from: startOfWeek(now, offsetMin), to: null };
  if (range === "mtd") return { from: startOfMonth(now, offsetMin), to: null };
  if (RANGE_MS[range]) return { from: new Date(now.getTime() - RANGE_MS[range]), to: null };
  return none;
}

/** Where a range begins — kept for callers that only care about the start. */
export const rangeStart = (range, now = new Date(), offsetMin = 0) =>
  rangeBounds(range, now, offsetMin).from;

function inBounds(closeTime, from, to) {
  if (!closeTime) return false;
  const t = new Date(closeTime);
...
export function filterDealsByRange(deals, range, now = new Date(), offsetMin = 0) {
  const { from, to } = rangeBounds(range, now, offsetMin);
  if (!from && !to) return deals;
  return deals.filter((d) => inBounds(d.closeTime, from, to));
}
...
export function buildModel(snapshot, { range = "all", symbols = null, offsetMin = 0 } = {}) {
  ...
  const now = new Date();
  const { from, to } = rangeBounds(range, now, offsetMin);
  let deals = from || to ? allDeals.filter((d) => inBounds(d.closeTime, from, to)) : allDeals;
  ...
  // Floating P/L belongs to the period that is still running — a range that
  // already ended never saw today's open trades, so it must not count them.
  const openEnded = !to || to > now;
  const growth = base > 0 ? round2(((portfolio.net + (openEnded ? floating : 0)) / base) * 100) : 0;
