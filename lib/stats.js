 ...
 */
export function statsFor(deals, base = 0, ddDenominator = 0) {
  ...
  for (const d of deals) {
    ...
    w.net += netOf(d);
    w.trades += 1;
    const key = d.closeTime.slice(0, 10);
...
export function filterDealsByRange(deals, range, now = new Date()) {
  if (!range || range === "all") return deals;
  let from;
  if (range === "today") {
    from = new Date(now);
 ...
 */
export function buildModel(snapshot, { range = "all", symbols = null } = {}) {
  const s = snapshot || { account: {}, deals: [], positions: [], equityHistory: [] };
  let deals = filterDealsByRange(s.deals || [], range);
  ...
  const portfolio = statsFor(deals, initial);
  ...
  const bySymbol = names.map((name) => {
    ...
    const st = statsFor(sd, 0, initial);
  ...
  const growth = initial > 0 ? round2(((account.equity - initial) / initial) * 100) : 0;
