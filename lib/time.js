...
/** Real-UTC instant at which a market day ("YYYY-MM-DD") starts. */
export function dayStart(key, offsetMin) {
  const ms = Date.parse(`${key}T00:00:00Z`);
  return Number.isNaN(ms) ? null : new Date(ms - offsetMin * 60000);
}

/** Real-UTC instant at which a market day ends — i.e. the next day's start. */
export function dayEnd(key, offsetMin) {
  const start = dayStart(key, offsetMin);
  return start ? new Date(start.getTime() + 864e5) : null;
}
